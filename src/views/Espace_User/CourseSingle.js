...
const CourseSingle = (params) => {
  ...
  const RegisterComment = async (event) => {
    ...
    try {
      ...
      try {
        const response8 = await axios.get('http://127.0.0.1:8000/api/CommentCoursesAvg/1', {
          headers: {}
        });
        ...
        if (response8.status === 200) {
          ...
          try {
            await axios.put('http://127.0.0.1:8000/api/updateRatCourse/1', { rating });
                      ...
                      <div className="d-flex align-items-center mb-6">
                        <div className="d-inline-block rounded-circle border me-6 p-2">
                          <div className="avatar avatar-size-120">
                            <img src={`../${course.user && course.user.image }`} alt="..." className="avatar-img rounded-circle" />
                          </div>
                        </div>
